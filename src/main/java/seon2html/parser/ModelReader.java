...
public class ModelReader {
	...
	public Package parseAstah2Seon(String filename) {
		...
		try {
			...
			seon = new Package(model.getName(), model.getDefinition(), PackType.NETWORK, 0, model);
		...
		for (INamedElement elem : superpack.getAstahPack().getOwnedElements()) {
			...
			if (elem instanceof IPackage) {
				...
				if (type != PackType.IGNORE) {
					...
					int order = 100;
					if (!sorder.isEmpty()) order = Integer.parseInt(sorder);

					// Creates a Package or an Ontology
