...
public class PageWriter {
	...
	private void generateSeonPage(Package seon) {
		...
		Diagram diagram = seon.getDiagrams().getFirst(); // suposing only one package diagram here.
	...
	private void generateNetworkGraph(Package seon) {
		...
		List<Ontology> nwOntos = new ArrayList<>();
		...
		for (Ontology ontology : nwOntos) {
			...
			nodes.append("  {data: {id:'").append(id).append("', name:'").append(name).append("', dim:").append(diff + (int) (factor * ontology.getAllConcepts().size())).append(", level:").append(level).append(", color:'").append(color).append("'}},\n");
			id++;
		}
		...
		for (int i = 0; i < depMatrix.length; i++) {
			for (int j = 0; j < depMatrix.length; j++) {
				...
				if (i != j && weight > 0) {
					...
					edges.append("  {data: {id:'e").append(id).append("', thickness:").append(thickness).append(", weight:").append(weight).append(", source:'").append(i).append("', target:'").append(j).append("'}},\n");
					id++;
		...
		// Replacing the tags
		code = code.replace("@nodes", nodes.toString());
		code = code.replace("@edges", edges.toString());
	...
	private void generateMenuPage() {
		...
		for (Ontology ontology : ontologies) {
			...
			if (level != null) {
				...
				if (level == OntoLevel.FOUNDATIONAL) found.append(line).append("\n");
				else if (level == OntoLevel.CORE) core.append(line).append("\n");
				else if (level == OntoLevel.DOMAIN) domain.append(line).append("\n");
				// other level: ignore
			}
		}
		html = html.replace("@foundOntology", found.toString());
		html = html.replace("@coreOntologies", core.toString());
		html = html.replace("@domainOntologies", domain.toString());
	...
	private void generateStatsPage() {
		...
		for (Ontology ontology : ontologies) {
			...
			if (depends > 0) {
				allDeps = new StringBuilder("(");
				for (int i = 0; i < ontoDeps.length; i++) {
					if (ontoDeps[i] > 0) {
						allDeps.append(ontologies.get(i).getShortName()).append(":").append(ontoDeps[i]).append(", ");
					}
				}
				allDeps = new StringBuilder(allDeps.substring(0, allDeps.length() - 2) + ")");
			}
			StringBuilder allGens = new StringBuilder();
			if (generals > 0) {
				allGens = new StringBuilder("(");
				for (int i = 0; i < ontoGens.length; i++) {
					if (ontoGens[i] > 0) {
						allGens.append(ontologies.get(i).getShortName()).append(":").append(ontoGens[i]).append(", ");
					}
				}
				allGens = new StringBuilder(allGens.substring(0, allGens.length() - 2) + ")");
			}
			...
			if (level != null) {
				if (level == OntoLevel.FOUNDATIONAL) found.append(stats);
				else if (level == OntoLevel.CORE) core.append(stats);
				else if (level == OntoLevel.DOMAIN) domain.append(stats);
				// other level: ignore
			}
		}
		html = html.replace("@foundOntology", found.toString());
	...
	private void generateSearchBase() {
		StringBuilder conceptsHash = new StringBuilder("var concepts = {\n");
		List<Concept> concepts = Concept.getAllConcepts();
		Collections.sort(concepts);
		for (Concept concept : concepts) {
			String definition = concept.getDefinition().replaceAll("'", "").replaceAll("\n", ". ");
			conceptsHash.append("'").append(concept.getName()).append("': {'def': '").append(definition).append("', 'ref': '").append(concept.getReference()).append("_detail'},\n");
		}
		conceptsHash.append("};");
		Utils.stringToFile("./page/ConceptsHash.js", conceptsHash.toString());
	...
	private String generateDependenciesTable(Ontology onto) {
		...
		StringBuilder dependsTable = new StringBuilder();
		for (Dependency depend : onto.getDependencies()) {
			...
			dependsTable.append(line).append("\n");
		}
		return dependsTable.toString();
	}
	...
	private String generateSectionStructures(Package superpack, String snum) {
		...
		for (Package pack : packs) {
			...
			// recursive call
			sectionStructures.append(struct).append(generateSectionStructures(pack, (snum + num + ".")));
			num++;
		}
	...
	private String parseMap(Diagram diagram) {
		...
		StringBuilder mapcode = new StringBuilder("<MAP NAME=\"" + diagram.getName() + "\">");
		IDiagram aDiagram = diagram.getAstahDiagram();
		try {
			...
			if (diagram.getType() == DiagType.CONCEPTUALMODEL) {
				...
				for (IPresentation present : aDiagram.getPresentations()) {
					if (present instanceof INodePresentation node && present.getType().equals("Class")) {
				...
				// last.
				Comparator<IPresentation> comp = (p1, p2) -> {
	...
	private void recoverStaticPages() {
		...
		try {
			...
			// replacing the top page if it is an instable version
	...
	public String generateConceptsTable(Ontology onto) {
		...
		for (Concept concept : concepts) {
			...
			conceptsTable.append(line).append("\n");
		}
	...
	private String generateDetailedConcepts(Ontology onto) {
		...
		for (Concept concept : concepts) {
			...
			StringBuilder relats = new StringBuilder();
			if (!relations.isEmpty()) {
				relats = new StringBuilder("Relations:<br/>\n<code>");
				for (Relation relation : relations) {
					...
					if (SeonParser.STABLE || ontoSource.getLevel().getValue() >= ontoTarget.getLevel().getValue()) {
						relats.append(relation).append("<br/>\n");
					} else {
						relats.append("<span style='color:red' title='Relation to a lower level (").append(ontoSource.getName()).append("-->").append(ontoTarget.getName()).append(")'>").append(relation).append("</span><br/>\n");
					}
				}
				relats.append("</code>");
			}
			item = item.replace("@relations", relats.toString());

			detailedConcepts.append(item).append("\n\n");
		}
		return detailedConcepts.toString();
	}

	/** Formats a description to appear in an HTML page. */
